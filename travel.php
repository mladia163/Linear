...
$com="SELECT cid from comp where chk=0";
 ...
 $tqr="SELECT * from travel where cid='$cid'";
...
?>  
  ...
  function fun1(x){
	...
	$ans=mysql_fetch_assoc($ansq);				
					
	?>
                    <div id="anss" style="display:none"><ul>
                <?php do{ ?> <li> -> <?php 
					 echo $ans['ans'];
					 echo "\n";
					//echo "yooo";
					...
					?></li><br>
             <?php }while($ans=mysql_fetch_assoc($ansqr)); ?> 
             <li><form action="food.php" method="post"><input type="text" name="comm" id="tt"> <button type="submit">Comment</button></form></li> 
             </ul>    </div>
